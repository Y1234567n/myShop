<template>
	<div class="demo1">
		<list-pull-loading :options="options" ref="listPullLoading">
			<template slot="list">
				<ul class="media-list">
					<li v-for="(dataItem,dataIndex) in dataList" :key="dataItem.id||dataIndex" class="media-list-box">
						<a class="item-content">1234
  ...
  import "list-pull-loading/dist/list-pull-loading.css"
    ...
    components:{
       "list-pull-loading": listPullLoading
		...
		created(){
			let result = require("../../mockData/list1.json").result;
			for(let i = 0; i < result.items.length; i++){
				result.items[i].userPic = require("../../imgs/banner.jpg");
			}
			console.log(result)
			this.allData = result;
		},
